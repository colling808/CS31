...
void Snake::move()
{
    ...
    directionToDeltas(rand() % 4, rowDelta, colDelta);
    
    if (m_row + rowDelta <= m_pit->rows() && m_row + rowDelta > 0 && m_col + colDelta <= m_pit->cols() && m_col + colDelta > 0)
    {
        m_row += rowDelta;
        m_col += colDelta;
    }
...
void Player::move(int dir)
{
    ...
    directionToDeltas(dir, rowDelta, colDelta);
    ...
    if (m_row + rowDelta <= m_pit->rows() && m_row + rowDelta > 0 && m_col + colDelta <= m_pit->cols() && m_col + colDelta > 0)
    {
        ...
        m_col += colDelta;
        if (snakeExists)
            m_pit->destroyOneSnake(m_row - rowDelta / 2, m_col - colDelta / 2);
    }
        ...
        for (r = 0; r < rows(); r++)
            for (c = 0; c < cols(); c++)
            {
                if (numberOfSnakesAt(r+1, c+1) == 1)
                    grid[r][c] = 'S';
                if (numberOfSnakesAt(r+1, c+1) >= 2 && numberOfSnakesAt(r+1, c+1) <= 8)
                    grid[r][c] = char(numberOfSnakesAt(r+1, c+1) + '0');
